...
RSpec.describe 'BitmartFutures integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:market) { 'bitmart_futures' }
  let(:btc_usdt_pair) { Cryptoexchange::Models::MarketPair.new(base: 'BTC', target: 'USDT', market: market, inst_id: 1) }
  ...
  it 'give trade url' do
    ...
    expect(trade_page_url).to eq "https://futures.bitmart.com/?symbol=BTC_USDT"
  end
  ...
  context 'fetch contract stat' do
    it 'fetch contract stat' do
      ...
      expect(contract_stat.market).to eq 'bitmart_futures'
