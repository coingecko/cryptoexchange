...
RSpec.describe 'Big Markets integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:eth_btc_pair) { Cryptoexchange::Models::MarketPair.new(base: 'ETH', target: 'BTC', market: 'bigmarkets') }
  ...
  it 'has trade_page_url' do
    ...
    expect(trade_page_url).to eq "https://www.bigmarkets.io/dashboard"
  end

  it 'fetch pairs' do
    pairs = client.pairs('bigmarkets')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'bigmarkets'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'bigmarkets'
  ...
  #   expect(order_book.market).to eq 'bigmarkets'
  ...
  #   expect(trade.market).to eq 'bigmarkets'
