...
RSpec.describe 'Bithash integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:eth_btc_pair) { Cryptoexchange::Models::MarketPair.new(base: 'eth', target: 'btc', market: 'bithash') }

  it 'fetch pairs' do
    pairs = client.pairs('bithash')
    expect(pairs).not_to be_empty

    pair = pairs.sample
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'bithash'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'bithash'
  ...
  it 'fetch order book' do
    ...
    expect(order_book.market).to eq 'bithash'
  ...
  it 'fetch trade' do
    ...
    expect(trade.market).to eq 'bithash'
  end
end
