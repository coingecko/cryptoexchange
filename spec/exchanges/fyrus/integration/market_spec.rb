...
RSpec.describe 'Fyrus integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:bch_usdt_pair) { Cryptoexchange::Models::MarketPair.new(base: 'BCH', target: 'USDT', market: 'fyrus') }

  it 'fetch pairs' do
    pairs = client.pairs('fyrus')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'fyrus'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'fyrus'
  ...
  #   expect(order_book.market).to eq 'fyrus'
  ...
  #   expect(trade.market).to eq 'fyrus'
