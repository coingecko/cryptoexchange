...
RSpec.describe 'phemex integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:btc_usd_pair) { Cryptoexchange::Models::MarketPair.new(base: 'BTC', target: 'USD', market: 'phemex', contract_interval: "perpetual", inst_id: "BTCUSD") }

  it 'fetch pairs' do
    pairs = client.pairs('phemex')
    ...
    expect(pair.market).to eq 'phemex'
  end

  it 'give trade url' do
    trade_page_url = client.trade_page_url 'phemex', inst_id: btc_usd_pair.inst_id
    expect(trade_page_url).to eq 'https://phemex.com/trade/BTCUSD'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'phemex'
  ...
  it 'fetch order book' do
    ...
    expect(order_book.market).to eq 'phemex'
  ...
  it 'fetch contract stat' do
    ...
    expect(contract_stat.market).to eq 'phemex'
