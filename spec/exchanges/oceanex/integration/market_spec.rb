...
RSpec.describe 'Oceanex integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:btc_usdt_pair) { Cryptoexchange::Models::MarketPair.new(base: 'BTC', target: 'USDT', market: 'oceanex') }

  it 'fetch pairs' do
    pairs = client.pairs('oceanex')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'oceanex'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'oceanex'
  ...
  it 'fetch order book' do
    ...
    expect(order_book.market).to eq 'oceanex'
  ...
  it 'fetch trade' do
    ...
    expect(trade.market).to eq 'oceanex'
