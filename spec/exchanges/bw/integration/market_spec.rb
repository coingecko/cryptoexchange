...
RSpec.describe 'BW integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:eth_btc_pair) { Cryptoexchange::Models::MarketPair.new(base: 'ETH', target: 'BTC', market: 'bw') }
  ...
  it 'has trade_page_url' do
    ...
    expect(trade_page_url).to eq "https://www.bw.com/trade/eth_btc"
  end

  it 'fetch pairs' do
    pairs = client.pairs('bw')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'bw'
  end
  ...
  #   expect(order_book.market).to eq 'bw'
