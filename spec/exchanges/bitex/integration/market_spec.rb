...
RSpec.describe 'Bitex.la integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:btc_usd_pair) { Cryptoexchange::Models::MarketPair.new(base: 'BTC', target: 'USD', market: 'bitex') }

  it 'fetch pairs' do
    pairs = client.pairs('bitex')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'bitex'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'bitex'
  ...
  it 'fetch order book' do
    ...
    expect(order_book.market).to eq 'bitex'
  ...
  #   expect(trade.market).to eq 'bitex'
  # end
end
