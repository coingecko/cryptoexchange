...
RSpec.describe 'AlienCloud integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:eth_btc_pair) { Cryptoexchange::Models::MarketPair.new(base: 'ETH', target: 'BTC', market: 'alien_cloud') }
  ...
  it 'has trade_page_url' do
    ...
    expect(trade_page_url).to eq "https://aliencloud.xyz/en/trade/eth_btc"
  end

  it 'fetch pairs' do
    pairs = client.pairs('alien_cloud')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'alien_cloud'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'alien_cloud'
  ...
  it 'fetch order book' do
    ...
    expect(order_book.market).to eq 'alien_cloud'
  ...
  it 'fetch trade' do
    ...
    expect(trade.market).to eq 'alien_cloud'
