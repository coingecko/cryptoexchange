...
RSpec.describe 'Bitnaru integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:btc_krw_pair) { Cryptoexchange::Models::MarketPair.new(base: 'BTC', target: 'KRW', market: 'bitnaru') }
  ...
  it 'has trade_page_url' do
    ...
    expect(trade_page_url).to eq "https://www.bitnaru.com/exchange"
  end

  it 'fetch pairs' do
    pairs = client.pairs('bitnaru')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'bitnaru'
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'bitnaru'
  ...
  #   expect(order_book.market).to eq 'bitnaru'
  ...
  #   expect(trade.market).to eq 'bitnaru'
