...
RSpec.describe 'Vcc integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:market) { 'vcc' }
  let(:eth_btc_pair) { Cryptoexchange::Models::MarketPair.new(base: 'eth', target: 'btc', market: 'vcc') }

  it 'fetch pairs' do
    pairs = client.pairs('vcc')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'vcc'
  end

  it 'give trade url' do
    trade_page_url = client.trade_page_url market, base: eth_btc_pair.base, target: eth_btc_pair.target
    expect(trade_page_url).to eq "https://vcc.exchange/exchange/basic?currency=btc&coin=eth"
  end
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'vcc'
