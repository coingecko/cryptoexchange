...
RSpec.describe 'GmoJapan integration specs' do
  let(:client) { Cryptoexchange::Client.new }
  let(:btc_jpy_pair) { Cryptoexchange::Models::MarketPair.new(base: 'BTC', target: 'JPY', market: 'gmo_japan') }

  it 'fetch pairs' do
    pairs = client.pairs('gmo_japan')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'gmo_japan'
  end

  it 'give trade url' do
    trade_page_url = client.trade_page_url 'gmo_japan', base: btc_jpy_pair.base, target: btc_jpy_pair.target
  ...
  it 'fetch ticker' do
    ...
    expect(ticker.market).to eq 'gmo_japan'
  ...
  it 'fetch order book' do
    ...
    expect(order_book.market).to eq 'gmo_japan'
  ...
  it 'fetch trade' do
    ...
    expect(trade.market).to eq 'gmo_japan'
