...
RSpec.describe 'Zoomex integration specs' do
  ...
  it 'fetch pairs' do
    pairs = client.pairs('zoomex')
    expect(pairs).not_to be_empty

    pair = pairs.first
    expect(pair.base).to_not be nil
    expect(pair.target).to_not be nil
    expect(pair.market).to eq 'zoomex'
  end
  ...
  it 'give trade url' do
    ...
    expect(trade_page_url).to eq "https://www.zooomex.com/exchange/#{btc_usdt_pair.base}-#{btc_usdt_pair.target}"
  end
  ...
  it 'fetch order book' do
    ...
    expect(order_book.market).to eq 'zoomex'
  ...
  it 'fetch trade' do
    ...
    expect(trade.market).to eq 'zoomex'
